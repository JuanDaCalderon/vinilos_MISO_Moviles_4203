...
class CacheManager(context: Context) {
    ...
    private var coleccionistas: HashMap<Int, Collector> = hashMapOf<Int, Collector>()
    ...
    fun addCollectors(collectorId: Int, collectorParam: Collector) {
        if (!coleccionistas.containsKey(collectorId)) {
            coleccionistas[collectorId] = collectorParam
        }
    }
    fun getCollectors(collectorId: Int): Collector? {
        return if (coleccionistas.containsKey(collectorId)) coleccionistas[collectorId]!! else null
    }
}